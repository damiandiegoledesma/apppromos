...
export function renderDashboard(container, businessId, meta, state, options = {}) {
  ...
  container.innerHTML = `
    <style>
      ...
      @media (max-width: 900px) { .dash-actions, .dash-grid, .dash-two { grid-template-columns:1fr; } }
      @media (max-width: 640px) { .dash-main-card { padding:18px; } .dash-main-title { font-size:30px; } .dash-value { font-size:26px; } .dash-action-btn { min-height:auto; padding:16px; } .dash-business-head { align-items:flex-start; flex-direction:column; } .dash-list-item { flex-direction:column; gap:4px; } .dash-list-item strong { text-align:left; } .dash-form-actions { flex-direction:column-reverse; } .dash-mini-btn, .dash-save-btn { width:100%; } }
    </style>
