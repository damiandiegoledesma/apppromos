...
function getPanelCopy(panelId) {
  if (panelId === "pricesPanel") {
    return {
      message: "Estás en Precios. Ajustá lo necesario y después pasá a Ofertas.",
      hint: "El camino corto es: precio claro, oferta rápida y WhatsApp.",
  ...
  if (panelId === "builderPanel") {
    return {
      message: "Estás armando una oferta. Cuando la tengas, mandala por WhatsApp.",
      hint: "Oferta simple, precio claro y mensaje listo para vender.",
  ...
  if (panelId === "whatsappPanel") {
    return {
      message: "Estás en WhatsApp. Elegí la oferta y compartila con tus clientes.",
      hint: "Acá se cierra el circuito: Precios → Ofertas → WhatsApp.",
...
function getCarnizaCopy() {
  ...
  if (access.level === "blocked") {
    return {
      badge: "🔒 Pausado",
      tone: "blocked",
      message: `El acceso operativo de ${businessName} está pausado.`,
      hint: "Escribinos y te ayudamos a reactivarlo. Siempre dejá al cliente con salida clara.",
  ...
  if (access.level === "warning") {
    ...
    return {
      ...
      hint: "Carniza te avisa para que no pierdas tiempo buscando por qué no guarda.",
  ...
  if (commercialKey === "trial_ending_soon") {
    return {
      ...
      hint: "Seguí vendiendo normal. Si querés, te muestro cómo dejarlo activo.",
  ...
  if (access.level === "trial") {
    ...
    return {
      badge: "🎁 Prueba",
      tone: "trial",
      message: panelCopy?.message || "Estás probando AppPromos. Aprovechá: armá una oferta y mandala por WhatsApp.",
      hint: panelCopy?.hint || "Probalo como se usa en el mostrador: Precios, Ofertas y WhatsApp.",
  ...
  return {
    ...
    hint: panelCopy?.hint || "Te llevo por el camino corto: Precios → Ofertas → WhatsApp.",
...
function renderCarniza() {
  ...
  carnizaPanel.innerHTML = `
    <div class="carniza-card-head">
      <div class="carniza-title-wrap">
        ...
        <div>
          ...
          <p class="carniza-subtitle">Vendedor de AppPromos</p>
        </div>
      </div>
    ...
    <div class="carniza-footer">Carniza no cambia datos sola: te guía para vender o regularizar.</div>
  `;
}
