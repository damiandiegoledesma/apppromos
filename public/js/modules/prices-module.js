...
export function renderPrices(container, products = [], businessId = null, options = {}) {
  ...
  function refreshIdleStatus() {
    ...
    if (pendingCount > 0) {
      ...
      setStatus("error", "🔒 Guardado pausado por estado de cuenta");
    } else {
  ...
  function blockWriteAttempt() {
    setStatus("error", "🔒 Guardado pausado por estado de cuenta");
    showToast("Guardado pausado: regularizá por WhatsApp", "warn");
    return false;
  }
  ...
  container.innerHTML = `
    ...
    <div class="prices-shell">
      ...
      ${!canWrite ? `<div style="padding:14px 16px;border:1px solid #f97316;border-radius:16px;background:#fff4e5;color:#9a3412;font-weight:900;line-height:1.35;">🔒 Modo consulta: podés ver la lista de precios, pero el guardado está pausado hasta regularizar la cuenta.</div>` : ""}
