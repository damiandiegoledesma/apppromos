...
import { createDemoBusinessPayload } from "../data/demo-business-data.js";
...
export async function openBusiness(businessId, options = {}) {
  const sessionForDemo = await resolveSession().catch(() => null);
  if (sessionForDemo?.isDemo) {
    const demoPayload = createDemoBusinessPayload();
    setBusinessStore(demoPayload, { source: "demo-memory" });
    return demoPayload;
  }
...
export async function updateBusinessState(businessId, partialState) {
  ...
  const sessionForWrite = await resolveSession().catch(() => null);
  if (sessionForWrite?.isDemo) {
