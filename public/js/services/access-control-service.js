...
export function getAccessState(business = {}) {
  ...
  if (restricted) {
    ...
    return {
      ...
      showWarning: true,
      title: suspendedByPayment ? "Cuenta suspendida temporalmente" : "Acceso suspendido",
      message: suspendedByPayment
        ? "Tu cuenta está suspendida momentáneamente. Queremos ayudarte a reactivarla cuanto antes. Escribinos por WhatsApp y lo resolvemos."
        : "El acceso operativo de esta carnicería está suspendido desde Administración. Podés ingresar para ver el estado, pero los módulos quedan pausados.",
      ctaLabel: "Contactar AppPromos",
  ...
  if (paymentOverdue) {
    return {
      ...
      showWarning: true,
      title: trialExpired ? "Tu prueba finalizó" : "Pago pendiente",
      message: trialExpired
        ? "Tu período de prueba finalizó. Podés seguir entrando y consultando tu información. Para volver a guardar cambios y continuar usando AppPromos normalmente, activá tu plan."
        : "Detectamos un vencimiento pendiente en tu plan. Podés seguir entrando y consultando la app. Para volver a guardar cambios, regularizá tu cuenta por WhatsApp.",
      ctaLabel: trialExpired ? "Activar plan por WhatsApp" : "Regularizar por WhatsApp",
...
export function renderAccessWarning(business = {}) {
  ...
  if (!access.showWarning) return "";

  const styles = {
    blocked: { color: "#b42318", bg: "#fff1f0", border: "#f04438", icon: "⛔" },
    warning: { color: "#9a3412", bg: "#fff4e5", border: "#f97316", icon: "⚠️" },
    trial: { color: "#6b4b3e", bg: "#fff8f4", border: "#d7a86e", icon: "⏳" }
  };
  const style = styles[access.level] || styles.trial;
  const showCta = Boolean(access.ctaLabel && access.ctaUrl);

  return `
    <div data-access-warning="true" data-access-level="${escapeHtml(access.level)}" style="margin:0 0 14px;padding:16px 18px;border:2px solid ${style.border};border-radius:16px;background:${style.bg};color:${style.color};box-shadow:0 10px 26px rgba(0,0,0,.06);">
      <div style="display:flex;gap:10px;align-items:flex-start;">
        <div style="font-size:24px;line-height:1;">${style.icon}</div>
        <div style="flex:1;min-width:0;">
          <div style="font-weight:1000;margin-bottom:4px;font-size:16px;">${escapeHtml(access.title)}</div>
          <div style="font-size:14px;line-height:1.38;font-weight:800;">${escapeHtml(access.message)}</div>
          ${showCta ? `
            <a href="${escapeHtml(access.ctaUrl)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;justify-content:center;margin-top:12px;min-height:38px;padding:0 14px;border-radius:999px;background:#25D366;color:#10351f;text-decoration:none;font-weight:1000;box-shadow:0 4px 12px rgba(0,0,0,.12);">
              💬 ${escapeHtml(access.ctaLabel)}
            </a>
            <div style="margin-top:6px;font-size:12px;font-weight:800;opacity:.86;">WhatsApp AppPromos: ${escapeHtml(APPPROMOS_SUPPORT_PHONE_DISPLAY)}</div>
          ` : ""}
        </div>
      </div>
    </div>
...
export function getBusinessWriteBlockReason(business = {}) {
  ...
  if (access.level === "blocked") {
    return access.message || "La cuenta está pausada. Podés consultar la información, pero no guardar cambios.";
  }

  if (access.level === "warning") {
    return access.message || "La cuenta requiere regularización. Podés consultar la información, pero no guardar cambios.";
  }
